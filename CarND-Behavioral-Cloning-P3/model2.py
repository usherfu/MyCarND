...
def train_model(model, args, X_train, X_valid, y_train, y_valid):
    ...
    model.fit_generator(batch_generator(args.data_dir, X_train, y_train, args.batch_size, True),
                        ...
                        steps_per_epoch=int(len(X_train) / args.batch_size / 8),
                        epochs=args.nb_epoch, verbose=1, callbacks=[checkpoint, earlystopping],
                        validation_data=batch_generator(args.data_dir, X_valid, y_valid, args.batch_size, False),
                        validation_steps=int(len(X_valid) / args.batch_size / 8))
...
def main():
    ...
    parser.add_argument('-d', help='data directory',        dest='data_dir',          type=str,   default='/data/udacity_data')
