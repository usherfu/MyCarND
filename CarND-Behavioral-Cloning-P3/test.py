...
import argparse
import random
from utils import load_image, resize, crop, rgb2yuv, preprocess, overlay_steering, STRAIGHT_STEERING
from utils import random_flip, random_translate, random_shadow, random_brightness
...
def main():
    ...
    Random test program
    """
    data_dir = '/data/testtrack_train_all'
    ...
    # Visualize preprocess() steps: original, cropped, resize and rgb2yuv
    img_cropped = crop(img_brightness)
    print("cropped:")
    ...
    cv2.imshow('resized, yuv', np.hstack((img_resize, img_yuv)))
    ...
    for i in range(200):
        idx = random.randint(0, len(steers)-1)
        if abs(steers[idx] < 0.5):
            continue

        center_angle = steers[idx]
        left_angle  = center_angle + 0.25
        right_angle = center_angle - 0.25

        center_img  = overlay_steering(preprocess(load_image(data_dir, centers[idx]), False), center_angle)
        left_img    = overlay_steering(preprocess(load_image(data_dir, lefts[idx]), False), left_angle, center_angle)
        right_img   = overlay_steering(preprocess(load_image(data_dir, rights[idx]), False), right_angle, center_angle)

        all_imgs = np.hstack((np.hstack((left_img, center_img)), right_img))
